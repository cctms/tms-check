    ...
    if search_query:
        ...
        if not search_results.empty:
            ...
            if selected_option != "선택하세요":
                ...
                st.divider()
                st.markdown(f"""<div style="font-size: 1.6rem; font-weight: 700; padding: 10px 0px; border-bottom: 2px solid #F0F2F6; margin-bottom: 20px;">
                ...
                with col2:
                    ...
                    # '외관 및 구조'에 속하는 실제 엑셀 시트 이름들
                    structure_sheets = ["시료채취지점", "측정소 입지조건", "측정소 구조 및 설비"]
                    ...
                    if found_check:
                        ...
                        for i, name in enumerate(check_names):
                            if is_checked(target_row.iloc[11 + i]):
                                if name == "외관 및 구조":
                                    # 외관 및 구조일 경우 세부 시트들을 루프 돌며 출력
                                    for s_name in structure_sheets:
