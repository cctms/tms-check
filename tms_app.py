...
def load_all_data():
    try:
        ...
        if not guide_path:
            st.error(f"❌ 가이드북 파일을 찾을 수 없습니다. 현재 파일: {files}")
    ...
    if search_query:
        ...
        if not search_results.empty:
            ...
            if selected_option != "선택하세요":
                ...
                with col3:
                    st.markdown("#### 📊 3. 상대정확도")
                    # 가이드북 엑셀의 22번 인덱스(W열 부근) 체크 확인
                    if is_checked(target_row.iloc[22]):
                        st.error("📍 수행 대상")
                        if rel_sheets:
                            # 첫 번째 시트나 '상대정확도' 포함 시트 로드
                ...
                if all_data_frames:
                    ...
                    st.download_button(label="📥 전체 결과 엑셀 다운로드", data=output.getvalue(),
                                       file_name=f"TMS_시험항목_{selected_sub}.xlsx",
                                       mime="application/
